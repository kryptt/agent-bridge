...
const MQTT_URL = process.env.MQTT_URL ?? 'mqtt://mosquitto.home:1883'
const MQTT_USER = process.env.MQTT_USER ?? ''
const MQTT_PASS = process.env.MQTT_PASS ?? ''
const MQTT_CLIENT_ID = `openclaw-mqtt-plugin-${crypto.randomUUID().slice(0, 8)}`
...
function getSingletonClient (): Promise<MqttClientType> {
  ...
  singletonReady = new Promise<MqttClientType>((resolve, reject) => {
    ...
    client.on('connect', () => {
      if (!singletonClient) console.log('[openclaw-plugin] Connected to MQTT broker')
    ...
    client.on('message', (topic, payload) => {
      for (const handler of handlers.values()) {
        try { handler(topic, payload) } catch (err) {
          console.error('[openclaw-plugin] Message handler error:', err)
        }
      }
    })

    client.on('error', (err) => console.error('[openclaw-plugin] MQTT error:', err.message))
    client.on('reconnect', () => { if (!singletonClient) console.log('[openclaw-plugin] Reconnecting...') })

    setTimeout(() => {
      if (!singletonClient) reject(new Error('MQTT connection timeout after 10s'))
    }, 10_000)
  })
      ...
      Effect.tryPromise({
        try: () => new Promise<void>((res, rej) => {
          client.subscribe(topicFilter, { qos: 1 }, (err) => {
            if (err) rej(err)
            else { handlers.set(topicFilter, handler); res() }
          })
        }),

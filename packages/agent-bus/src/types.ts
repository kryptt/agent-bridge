...
export interface MessageBase {
  ...
  readonly conversationId?: string  // groups related messages into a logical thread
}
...
/** Agent identification heartbeat — sent every 15 min, offline after TTL */
export interface HeartbeatMessage extends MessageBase {
  readonly type: 'heartbeat'
  readonly agentId: string             // session UUID, stable for lifetime of agent
  readonly agentType: string           // "claudecode" | "openclaw" | etc.
  readonly agentPurpose: string        // what the agent is responsible for
  readonly agentCapabilities: string[] // skills, commands, etc. available
  readonly activeHuman?: {
    readonly id: string                // canonical human id
    readonly lastInteraction: string   // ISO 8601 — when human last interacted with agent
  }
}

export const HEARTBEAT_TTL_MS = 15 * 60_000 // 15 minutes

export type AgentBusMessage = TaskMessage | ChatMessage | PresenceMessage | EventMessage | HeartbeatMessage
